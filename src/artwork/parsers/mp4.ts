...
async function findCoverArtData(
  ...
  const meta = await findAtom(
    reader,
    udta.dataOffset,
    udta.dataOffset + udta.dataSize,
    'meta'
  );
  if (!meta) {
    return null;
  ...
  const covr = await findAtom(
    reader,
    ilst.dataOffset,
    ilst.dataOffset + ilst.dataSize,
    'covr'
  );
  if (!covr) {
    return null;
  ...
  const data = await findAtom(
    reader,
    covr.dataOffset,
    covr.dataOffset + covr.dataSize,
    'data'
  );
