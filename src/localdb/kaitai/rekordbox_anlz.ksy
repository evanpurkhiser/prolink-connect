  ...
  cue_extended_entry:
    ...
    seq:
      ...
      - id: loop_numerator
        type: u2
        doc: |
          The numerator of the loop size fraction for a quantized loop,
          e.g. 4 for a 4-beat loop. Zero when the loop is not quantized.
      - id: loop_denominator
        type: u2
        doc: |
          The denominator of the loop size fraction for a quantized loop,
          e.g. 1 for a 4-beat loop.
      - id: len_comment
        type: u4
        if: len_entry > 43
      - id: comment
